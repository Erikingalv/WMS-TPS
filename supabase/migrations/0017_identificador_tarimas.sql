-- Identificador de tarimas: rango físico (ej. "tarimas 1-20" o "81-100")
-- para poder rastrear cuáles entraron y cuáles salieron. Se guarda en
-- `lotes` (rango con el que llegó el lote) y por separado en cada
-- `entradas`/`salidas` (rango de ese movimiento puntual).

alter table public.lotes
  add column tarima_desde integer,
  add column tarima_hasta integer,
  add constraint lotes_rango_tarimas_valido
    check (tarima_desde is null or tarima_hasta is null or tarima_hasta >= tarima_desde);

alter table public.entradas
  add column tarima_desde integer,
  add column tarima_hasta integer,
  add constraint entradas_rango_tarimas_valido
    check (tarima_desde is null or tarima_hasta is null or tarima_hasta >= tarima_desde);

alter table public.salidas
  add column tarima_desde integer,
  add column tarima_hasta integer,
  add constraint salidas_rango_tarimas_valido
    check (tarima_desde is null or tarima_hasta is null or tarima_hasta >= tarima_desde);

-- ---------------------------------------------------------------
-- registrar_entrada: agrega el rango de tarimas de la entrada, se guarda
-- también en el lote (rango con el que llegó).
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- registrar_salida: agrega el rango de tarimas que salen en ese movimiento
-- puntual (no se toca el rango original del lote).
-- ---------------------------------------------------------------
