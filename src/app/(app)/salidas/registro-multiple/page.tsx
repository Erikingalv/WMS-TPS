...
export default async function RegistroMultipleSalidasPage({
  ...
  const { data: salidasRaw } =
    ids.length > 0
      ? await supabase
          ...
          .in("id", ids)
      : { data: [] as FilaResumen[] };
  const salidas = (salidasRaw ?? []) as unknown as FilaResumen[];
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-semibold text-ink">Salidas del viaje</h1>
        <p className="mt-1 text-sm text-ink-soft">
          {salidas.length} de {total} producto{total === 1 ? "" : "s"} se registraron correctamente.
        </p>
      </div>
            ...
            <Card key={s.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
              <div>
                <p className="text-sm font-medium text-ink">
                  {s.clientes?.nombre ?? "—"} · {s.productos?.nombre ?? "—"}{" "}
                  <span className="font-mono text-xs text-ink-faint">({s.productos?.sku})</span>
                </p>
                <p className="text-xs text-ink-faint">
                  {formatearNumero(s.cantidad_piezas)} pz · {formatearNumero(s.cantidad_tarimas)} tar · lote{" "}
      ...
      <div className="flex gap-3 pt-2">
        <ButtonLink href="/salidas/nueva">Registrar otro viaje</ButtonLink>
