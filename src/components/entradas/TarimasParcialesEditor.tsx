...
// Versión controlada (sin estado propio ni input oculto) de la edición de
// tarimas parciales — la usa TarimasParciales (página de editar una sola
// entrada, con su propio input oculto) y cada línea de EntradaForm cuando
// se capturan varios productos/clientes en una sola entrada consolidada.
export function TarimasParcialesEditor({
  value,
  onChange,
}: {
  value: TarimaParcial[];
  onChange: (parciales: TarimaParcial[]) => void;
}) {
  function agregar() {
    onChange([...value, { numero_tarima: null, piezas: 0 }]);
  }

  function actualizar(i: number, cambios: Partial<TarimaParcial>) {
    onChange(value.map((p, idx) => (idx === i ? { ...p, ...cambios } : p)));
  }
